...
from advlearn import BaseAttack, PoisonMixin, OptimizableAttackMixin
from advlearn import Projector
from sklearn.metrics import log_loss
import warnings


class SklearnAttack(BaseAttack, OptimizableAttackMixin, PoisonMixin):
    """A general poisoning attack against logistic regression."""

    def __init__(self, model, boundary=None, step_size=5, max_steps=5000, opt_method='GD', atol=1e-4):
        """A general poisoning attack against logistic regression.
        Parameters
        ----------
        boundary
    ...
    def attack_loss(self, attack_data, attack_label, extra_data=None, extra_labels=None):
        ...
        loss = log_loss(self.labels, probs)
        return loss
